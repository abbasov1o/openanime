...
/// One row of the Home screen's "Son Eklenen Bölümler" section: the show the
/// episode belongs to (as `Media`, so the existing card and detail flow render
/// it unchanged) plus the season/episode numbers for the badge. `addedAt`
/// keeps two episodes of the same show distinct in `ForEach`.
struct LatestEpisode: Codable, Equatable, Hashable, Identifiable {
    let media: Media
    let season: Int?
    let episode: Int?
    var addedAt: Double? = nil

    var id: String { "\(media.uniqueId)#\(season ?? -1)x\(episode ?? -1)#\(addedAt ?? 0)" }
}
...
struct HomeFeed: Codable {
    ...
    let popular: [Media]
    let topRated: [Media]
    /// Episodes most recently added on the provider, for the Home "latest episodes"
    /// row. Optional so caches written before the row existed still decode; `var`
    /// so the provider that fills it can default everyone else to nil.
    var latestEpisodes: [LatestEpisode]? = nil
}
