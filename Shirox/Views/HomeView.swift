                ...
                if vm.isLoading && vm.trending.isEmpty {
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 24) {
                            ...
                            #endif
                            if !vm.latestEpisodes.isEmpty {
                                LatestEpisodesSection(episodes: vm.latestEpisodes)
                            }
                            if !vm.trending.isEmpty {
...
// MARK: - Latest Episodes ("Son Eklenen Bölümler")

/// The provider's most recently added episodes, newest first. Cards open the
/// parent show's detail page, so playback flows through the same bridge as
/// every other card on the screen.
private struct LatestEpisodesSection: View {
...
private struct LatestEpisodeCard: View {
    let entry: LatestEpisode

    /// "S2 · B7", or the plain show name's fallback when the row carried no numbers.
    private var badgeText: String {
        switch (entry.season, entry.episode) {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ZStack(alignment: .bottomLeading) {
                CachedAsyncImage(urlString: entry.media.bannerImage ?? entry.media.coverImage.large ?? "")
                    .aspectRatio(16 / 9, contentMode: .fill)
                    .frame(width: 185, height: 104)
                    .clipped()
