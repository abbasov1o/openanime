...
final class UpcomingCalendarViewModel: ObservableObject {
    ...
    /// Whichever schedule the active provider can give.
    ///
    ...
    /// Anizium publishes its own release calendar with per-episode timestamps.
    private func fetchSchedule() async throws -> [AiringEpisode] {
        ...
        if ProviderManager.shared.primary?.providerType == .anizium {
            let days = try await AniziumService.shared.calendar()
            return days.compactMap { day in
                (day.contents ?? []).compactMap { entry -> AiringEpisode? in
                    guard let anime = entry.content, entry.onAir == true else { return nil }
                    let when = entry.releaseDate.map { Date(timeIntervalSince1970: $0 / 1000) }
                    guard let airingAt = when, airingAt >= start, airingAt <= end else { return nil }
                    return AiringEpisode(media: anime.toMedia(), episode: entry.episode, airingAt: airingAt)
                }
            }
            .flatMap { $0 }
        }
